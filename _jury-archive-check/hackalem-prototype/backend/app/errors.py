...
MESSAGES = {
    'invalid_edits': 'Проверьте поля правок: допустимы имена спикеров, текст и говорящий реплик, поля поручений и утверждение.',
    'entity_not_found': 'Реплика, спикер или поручение не найдено в этой встрече.',
    'result_not_ready': 'Правки и утверждение доступны после завершения анализа встречи.',
    'review_required': 'Проверьте все поручения и снимите отметки «требует проверки» перед утверждением.',
    'not_approved': 'Экспорт доступен только после утверждения готового результата человеком.',
    'libreoffice_missing': 'LibreOffice недоступен. Настройте LIBREOFFICE_PATH для экспорта PDF.',
    'pdf_conversion_failed': 'Не удалось создать PDF локальным LibreOffice. DOCX можно скачать отдельно.',
    'export_timeout': 'Превышено время локального преобразования в PDF. Повторите запрос позже.',
    'export_failed': 'Не удалось создать или сохранить документ в локальном хранилище.',
    'ollama_configuration': 'Для анализа разрешены только настроенный локальный Ollama и модель qwen3:8b. Проверьте режим запуска.',
